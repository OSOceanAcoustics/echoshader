...
class Echogram(param.Parameterized):
    ...
    def echograms(
            ...
            vmin: float = None, 
            vmax: float = None, 
            layout: Union[str("single_frequency"), 
                          str("multiple_frequency"), 
                          str("composite")] = "single_frequency", 
        ...
        if layout == "single_frequency":
            ...
            for index, channel in enumerate(self.MVBS_ds.channel.values):
                plot = holoviews.Dataset(self.MVBS_ds.sel(channel=channel)).to(holoviews.Image, 
                                vdims=["Sv"], 
            ...
            for ch, color in rgb_map.items():
                rgb_ch[color]=convert_to_color(self.MVBS_ds, 
                                            channel_sel=ch, 
                                            th_bottom=vmin, 
                                            th_top=vmax)
                    
            rgb = holoviews.RGB((self.MVBS_ds.ping_time.data, 
                                 self.MVBS_ds.echo_range.data,
                                rgb_ch["R"],
                                rgb_ch["G"],
                                rgb_ch["B"])).opts(self.gram_opts)
    ...
    def echogram(
        ...
        plot = holoviews.Dataset(self.MVBS_ds.sel(channel=self.channel_select.value)).to(holoviews.Image, 
                vdims=["Sv"], 
                kdims=["ping_time","echo_range"]
                ).opts(self.gram_opts)
