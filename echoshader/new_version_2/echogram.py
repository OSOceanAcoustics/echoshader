...
def echogram_multiple_frequency(
    ...
    This function takes an xarray.Dataset containing MVBS (Multibeam Backscatter) data,
    extracts data for the specified `channel` and generates echograms for that channel
    and other available frequency channels. Each echogram is a visual representation of
    the backscatter values (Sv) over time (ping_time) and depth (echo_range).
    ...
    MVBS_ds : xarray.Dataset
        xarray.Dataset containing MVBS data.
    channel : str
        The name of the frequency channel for which the echogram will be firstly generated.
        It should be a valid channel name present in the 'channel' dimension of MVBS_ds.
    cmap : str or List[str]
        The colormap(s) to use for the echograms. It can be a single colormap name or
        a list of colormap names for each frequency channel (if multiple colormaps are used).
    value_range : tuple[float, float]
        The minimum and maximum value for the color scale of the echogram.
    ...
    Returns
    -------
    holoviews.element.NdLayout
        A column of echograms for the specified frequency channel and other available
        frequency channels. Each echogram displays the backscatter values (Sv) over
        time (ping_time) and depth (echo_range). The echograms are rendered using Holoviews
        with the provided colormap and color scale limits.
    ...
    # Assuming MVBS_ds is an xarray.Dataset containing MVBS data
    # Generate multiple echograms for different frequency channels
    echogram_column = echogram_multiple_frequency(
        MVBS_ds,
        channel='GPT 38 kHz 00907208dd13 5-1 OOI.38|200',
        cmap='jet',
        value_range=(-80,-30)
    ...
    # Display the echogram grid using Panel
    Panel.Row(echogram_column)
    """
    gram_opts["Image"]["cmap"] = cmap
    ...
    for _, channel_ in enumerate(MVBS_ds.channel.values):
        ...
        echogram = (
            holoviews.Dataset(MVBS_ds.sel(channel=channel_))
            .to(holoviews.Image, vdims=["Sv"], kdims=["ping_time", "echo_range"])
            .opts(gram_opts)
        )
