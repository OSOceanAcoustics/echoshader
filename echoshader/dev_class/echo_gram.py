...
class Echogram(param.Parameterized):
    ...
    def _sync_widget(self):
        ...
        # Currently datetimeRangeSlider doestn't exist in panel ( v0.13.1, 2022-7-30 )
        ...
        self.color_map = panel.widgets.TextInput(name='Color Map',
                                                 placeholder='jet')
        ...
        self.range_clim = panel.widgets.EditableRangeSlider(
            ...
            value=(self.MVBS_ds.Sv.actual_range[0],
                   self.MVBS_ds.Sv.actual_range[-1]),
            step=0.01)
            
        # https://panel.holoviz.org/reference/layouts/WidgetBox.html#layouts-gallery-widgetbox
        
        self.widgets = panel.WidgetBox(str(self.lower_time) + \
                                     ' to ' + str(self.upper_time),
    ...
    @param.depends('datetime_range_input.value', 'time_range_picker.value',
                   'channel_select.value', 'color_map.value',
                   'range_clim.value')
    def view_gram(self):
        '''
        ...
        self.gram = self.MVBS_ds.Sv.sel(channel=channel,ping_time=time_range)\
            .hvplot(
        ...
        self.box = holoviews.streams.BoundsXY(
            source=self.gram,
            bounds=(self.MVBS_ds.sel(ping_time=time_range).ping_time.values[0],
                    self.MVBS_ds.echo_range.values[-1],
                    self.MVBS_ds.sel(
                        ping_time=time_range).ping_time.values[-1],
                    self.MVBS_ds.echo_range.values[0]))

        # plot box
        self.bounds = holoviews.DynamicMap(lambda bounds: holoviews.Bounds(
            bounds).opts(self.bound_opts), streams=[self.box])

        return self.gram * self.bounds

    @param.depends('datetime_range_input.value', 'time_range_picker.value',
                   'color_map.value', 'range_clim.value')
    def view_all_gram(self):
        ...
        self.all_gram = self.MVBS_ds.Sv.sel(ping_time=time_range,channel=self.MVBS_ds.channel.values[0])\
                .hvplot(
                kind= self.plot_types,
                x='ping_time',
                y='echo_range',
        ...
        for index, channel in enumerate(self.MVBS_ds.channel.values):
            ...
            self.all_gram += self.MVBS_ds.Sv.sel(ping_time=time_range,channel=channel)\
                .hvplot(
                kind= self.plot_types,
                x='ping_time',
                y='echo_range',
    ...
    def get_box_data(self):
        ...
        MVBS_ds : xarray.Dataset
        '''
        return self.MVBS_ds.sel(channel=self.channel_select.value,
                                ping_time=slice(self.box.bounds[0],
                                                self.box.bounds[2]),
                                echo_range=slice(self.box.bounds[3],
                                                 self.box.bounds[1]))
    ...
    def get_all_box_data(self):
        ...
        MVBS_ds : xarray.Dataset
        '''
        return self.MVBS_ds.sel(ping_time=slice(self.box.bounds[0],
                                                self.box.bounds[2]),
                                echo_range=slice(self.box.bounds[3],
                                                 self.box.bounds[1]))
...
def erase_error():
    ...
    '''
    erase_widget = panel.pane.HTML('''<script>code_show_err=false; 
