...
def plot_hist(echogram, MVBS, bin_size):
    ...
    box = streams.BoundsXY(
        source=echogram,
        bounds=(MVBS.ping_time.values[0], MVBS.echo_range.values[-1],
                MVBS.ping_time.values[-1], MVBS.echo_range.values[0]))
    ...
    def selected_box_table(bounds):
        # Apply current ranges
        obj_df = MVBS.sel(ping_time=slice(bounds[0], bounds[2]),
                          echo_range=slice(bounds[3],
                                           bounds[1])).Sv.to_dataframe()
    ...
    def get_box_data():
        return MVBS.sel(ping_time=slice(box.bounds[0], box.bounds[2]),
