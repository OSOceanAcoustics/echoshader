...
def simpleHist(ds_Sv, clim=(-80, -30), cmap="jet", width=600, height=300):
    """
...
def hist(ds_Sv, boxSelect=True, clim=(-80, -30), cmap="jet", width=600, height=300):
    """
    ...
    def selectedBoxTable(bounds):
        ...
        obj = (
            echogram.select(
                ping_time=(bounds[0], bounds[2]), echo_range=(bounds[1], bounds[3])
            )
            if bounds
            else echogram
        )
    ...
    bounds = hv.DynamicMap(
        lambda bounds: hv.Bounds(bounds).opts(line_width=1, line_color="white"),
        streams=[box],
    )
